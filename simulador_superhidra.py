...
class EstrategiaSuperHidra(EstrategiaBase):
    ...
    def generar_indicadores(self, df):
        ...
        tr_smooth = (
            ...
            .ewm(alpha=1 / self.adx_periodo, min_periods=self.adx_periodo)
        ...
        df["minus_di"] = 100 * (minus_dm_smooth.values / tr_smooth)
        ...
        df["adx"] = dx.ewm(
            ...
            alpha=1 / self.adx_periodo, min_periods=self.adx_periodo
        ...
        ema_200_4h_shifted = df_4h.ewm(span=200, adjust=False).mean().shift(1)
    ...
    def proximo_paso(self, vela):
        ...
        if self.posicion == "COMPRADO":
            ...
            if perdida_actual_real > 0.07:

                return "VENDER_STOP_LOSS"



            # 2. PROTECCIÓN BREAK EVEN (Si ganamos > 5%, no permitimos perder)

            if ganancia_actual_real > 0.05 and vela["low"] < self.precio_compra * 1.002:
            ...
            trailing_dinamico = 0.12  # 12% por defecto

            if ganancia_actual_real > 0.20:

                trailing_dinamico = 0.04  # Cosecha agresiva

            elif ganancia_actual_real > 0.10:

                trailing_dinamico = 0.07  # Asegura beneficios
